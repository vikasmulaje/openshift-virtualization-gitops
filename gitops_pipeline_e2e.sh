...
#      - Nightly builds: auto-fix image signature policy on nodes during install
#      - Nightly builds: apply MachineConfig + MCO recovery after install
...
#
#   Branch support (deploy from any git branch):
#   ./gitops_pipeline_e2e.sh --branch openshift-4.22            # Known branch (auto-config)
#   ./gitops_pipeline_e2e.sh --branch openshift-4.21 --clusters etl4
#   ./gitops_pipeline_e2e.sh --branch my-feature --network ocp3m0w-ic4n22 --ocp-image img4.22-custom
#
...
# to support arbitrary branches without editing the script
SUSHY_PORT="8000"
...
init_network_profile() {
  ...
  # libvirt network and subnet. Known branches have built-in defaults;
  # for any other branch, pass --network and --ocp-image (or env vars).
  case "${GITOPS_BRANCH}" in
    ...
    *)
      if [ -z "${LIBVIRT_NETWORK:-}" ] || [ -z "${OCP_VERSION_IMAGE:-}" ]; then
        log_error "Custom branch '${GITOPS_BRANCH}' requires --network and --ocp-image flags"
        log_error "  Example: $0 --branch ${GITOPS_BRANCH} --network <libvirt-net> --ocp-image <imageset-name>"
        log_error "  Or set env vars: LIBVIRT_NETWORK, OCP_VERSION_IMAGE"
        exit 1
      fi
...
phase1_setup_dns() {
  ...
  # 1. Add/update /etc/hosts entries on hypervisor
  ...
  for CLUSTER in $(get_deploy_clusters); do
    ...
    ssh_hyp "
      ...
      sed -i '/apps\.${!BASE_DOMAIN_VAR}/d' /etc/hosts
      echo '${!API_VIP_VAR} api.${!BASE_DOMAIN_VAR} api-int.${!BASE_DOMAIN_VAR}' >> /etc/hosts
      echo '${!INGRESS_VIP_VAR} apps.${!BASE_DOMAIN_VAR}' >> /etc/hosts
    "
  done
...
# ========================= NIGHTLY BUILD WORKAROUNDS ======================

is_nightly_build() {
  [[ "$OCP_VERSION_IMAGE" == *nightly* ]]
}

nightly_fix_node_policy() {
  local ip="$1"
  local INSECURE_POLICY='{"default":[{"type":"insecureAcceptAnything"}],"transports":{"docker-daemon":{"":[ {"type":"insecureAcceptAnything"}]}}}'

  ssh_hyp "
    timeout 10 ssh -o StrictHostKeyChecking=no -o ConnectTimeout=5 core@${ip} \
      'sudo python3 -c \"import json; json.dump(${INSECURE_POLICY}, open(\\\"/etc/containers/policy.json\\\",\\\"w\\\"), indent=2)\"' 2>/dev/null
  "
}

nightly_wait_for_node_services() {
  local ip="$1"
  local hostname="$2"
  local timeout=120
  local elapsed=0
  ...
  while [ $elapsed -lt $timeout ]; do
    local crio_active kubelet_active
    crio_active=$(ssh_hyp "timeout 5 ssh -o StrictHostKeyChecking=no -o ConnectTimeout=3 core@${ip} 'sudo systemctl is-active crio' 2>/dev/null" || echo "unknown")
    kubelet_active=$(ssh_hyp "timeout 5 ssh -o StrictHostKeyChecking=no -o ConnectTimeout=3 core@${ip} 'sudo systemctl is-active kubelet' 2>/dev/null" || echo "unknown")

    if [ "$crio_active" = "active" ] && [ "$kubelet_active" = "active" ]; then
      log_ok "  ${hostname} (${ip}): crio + kubelet active"
      return 0
    fi

    if [ "$crio_active" != "active" ] || [ "$kubelet_active" != "active" ]; then
      ssh_hyp "timeout 5 ssh -o StrictHostKeyChecking=no -o ConnectTimeout=3 core@${ip} 'sudo systemctl start kubelet' 2>/dev/null" || true
    fi
  ...
  log_warn "  ${hostname} (${ip}): services not active after ${timeout}s (crio=${crio_active}, kubelet=${kubelet_active})"
  return 1
}

phase3_nightly_fix_spoke_nodes() {
  if ! is_nightly_build; then
    return 0
  fi

  log_info "=== Nightly build detected: applying image signature policy workaround ==="

  for CLUSTER in $(get_deploy_clusters); do
    declare -n NODES="${CLUSTER^^}_NODES"
    local CLUSTER_KUBECONFIG="/tmp/${CLUSTER}-kubeconfig"

    log_info "Clearing stale SSH host keys for ${CLUSTER} nodes"
    for NODE in "${!NODES[@]}"; do
      local NODE_DATA="${NODES[$NODE]}"
      local IP=$(parse_node_field "$NODE_DATA" "IP")
      ssh_hyp "ssh-keygen -R ${IP} 2>/dev/null || true"
    done

    log_info "Fixing /etc/containers/policy.json on ${CLUSTER} nodes (insecureAcceptAnything)"
    for NODE in "${!NODES[@]}"; do
      local NODE_DATA="${NODES[$NODE]}"
      local IP=$(parse_node_field "$NODE_DATA" "IP")
      local HOSTNAME="${CLUSTER}-${NODE}"

      local reachable
      reachable=$(ssh_hyp "timeout 5 ssh -o StrictHostKeyChecking=no -o ConnectTimeout=3 core@${IP} 'echo ok' 2>/dev/null" || echo "")
      if [ "$reachable" != "ok" ]; then
        log_warn "  ${HOSTNAME} (${IP}): not reachable via SSH -- skipping"
        continue
      fi

      nightly_fix_node_policy "$IP"
      log_ok "  ${HOSTNAME} (${IP}): policy.json fixed"
    done

    log_info "Waiting for crio + kubelet to start on ${CLUSTER} nodes"
    for NODE in "${!NODES[@]}"; do
      local NODE_DATA="${NODES[$NODE]}"
      local IP=$(parse_node_field "$NODE_DATA" "IP")
      nightly_wait_for_node_services "$IP" "${CLUSTER}-${NODE}" || true
    done
  done
}
...
phase3_nightly_apply_mc_and_recover() {
  ...
  log_info "=== Nightly build: applying MachineConfig for insecure container policy ==="

  for CLUSTER in $(get_deploy_clusters); do
    local CLUSTER_KUBECONFIG="/tmp/${CLUSTER}-kubeconfig"

    local mc_exists
    mc_exists=$(ssh_hyp "KUBECONFIG=${CLUSTER_KUBECONFIG} oc get mc 99-master-nightly-container-policy --no-headers 2>/dev/null" || echo "")
    if [ -n "$mc_exists" ]; then
      log_ok "MachineConfig 99-master-nightly-container-policy already exists on ${CLUSTER}"
      continue
    fi
    ...
    ssh_hyp "KUBECONFIG=${CLUSTER_KUBECONFIG} oc apply -f - <<MCEOF
apiVersion: machineconfiguration.openshift.io/v1
kind: MachineConfig
metadata:
  labels:
    machineconfiguration.openshift.io/role: master
  name: 99-master-nightly-container-policy
spec:
  config:
    ignition:
      version: 3.2.0
...
MCEOF"
    log_ok "MachineConfig applied on ${CLUSTER}"

    log_info "Restoring original policy.json on ${CLUSTER} nodes to clear MCO degraded state"
    local OLD_MC
    OLD_MC=$(ssh_hyp "KUBECONFIG=${CLUSTER_KUBECONFIG} oc get mcp master -o jsonpath='{.status.configuration.name}' 2>/dev/null" || echo "")
    if [ -z "$OLD_MC" ]; then
      log_warn "Cannot determine current rendered MC -- skipping MCO recovery"
      continue
    fi

    ssh_hyp "KUBECONFIG=${CLUSTER_KUBECONFIG} oc get mc ${OLD_MC} -o json 2>/dev/null | python3 -c \"
import sys, json, base64, urllib.parse
data = json.load(sys.stdin)
files = data.get('spec',{}).get('config',{}).get('storage',{}).get('files',[])
for f in files:
    if f.get('path') == '/etc/containers/policy.json':
        src = f['contents']['source']
        parts = src.split(',', 1)
        if 'base64' in parts[0]:
            content = base64.b64decode(parts[1]).decode()
        else:
            content = urllib.parse.unquote(parts[1])
        print(content, end='')
        break
\" > /tmp/${CLUSTER}-original-policy.json"

    declare -n NODES="${CLUSTER^^}_NODES"
    for NODE in "${!NODES[@]}"; do
      local NODE_DATA="${NODES[$NODE]}"
      local IP=$(parse_node_field "$NODE_DATA" "IP")
      ssh_hyp "scp -o StrictHostKeyChecking=no -o ConnectTimeout=5 /tmp/${CLUSTER}-original-policy.json core@${IP}:/tmp/original-policy.json 2>/dev/null && \
        timeout 5 ssh -o StrictHostKeyChecking=no -o ConnectTimeout=3 core@${IP} 'sudo cp /tmp/original-policy.json /etc/containers/policy.json' 2>/dev/null" || true
    done
    log_ok "Original policy restored on ${CLUSTER} nodes"

    log_info "Waiting for MCO to reconcile on ${CLUSTER} (MCP updated + not degraded)"
    wait_for_condition "${CLUSTER} MCP master updated and not degraded" \
      "ssh_hyp 'KUBECONFIG=${CLUSTER_KUBECONFIG} oc get mcp master -o jsonpath=\"{.status.conditions}\"' 2>/dev/null | python3 -c \"
import sys, json
conds = json.load(sys.stdin)
updated = any(c['type']=='Updated' and c['status']=='True' for c in conds)
degraded = any(c['type']=='Degraded' and c['status']=='True' for c in conds)
sys.exit(0 if updated and not degraded else 1)
\" 2>/dev/null" \
      600 30

    log_ok "MCO healthy on ${CLUSTER}"
  done
}
...
phase3_nightly_monitor_mco_reboots() {
  ...
  log_info "=== Nightly build: monitoring MCO node reboots (starting kubelet if needed) ==="
  ...
  for CLUSTER in $(get_deploy_clusters); do
    ...
    while [ $elapsed -lt $MAX_WAIT ]; do
      ...
      for NODE in "${!NODES[@]}"; do
        ...
        local IP=$(parse_node_field "$NODE_DATA" "IP")
        local HOSTNAME="${CLUSTER}-${NODE}"
        local FQDN="${HOSTNAME}.${CLUSTER}.qe.lab.redhat.com"

        local node_ready
        node_ready=$(ssh_hyp "KUBECONFIG=${CLUSTER_KUBECONFIG} oc get node ${FQDN} -o jsonpath='{.status.conditions[?(@.type==\"Ready\")].status}' 2>/dev/null" || echo "Unknown")
      ...
      if [ "$all_ready" = true ]; then
        log_ok "All ${CLUSTER} nodes are Ready after MCO rollout"
        break
      fi
    ...
    if [ $elapsed -ge $MAX_WAIT ]; then
      log_warn "Timed out waiting for all ${CLUSTER} nodes to be Ready after MCO rollout"
    fi
  done
}
...
phase3_wait_spoke_provisioning() {
  ...
  for CLUSTER in $(get_deploy_clusters); do
    ...
    local NIGHTLY_FIX_APPLIED=false
    while [ $ELAPSED -lt $TIMEOUT ]; do
      ...
      if is_nightly_build && [ "$NIGHTLY_FIX_APPLIED" = false ] && [ $ELAPSED -ge 600 ]; then
        log_info "Nightly: attempting image signature policy fix on ${CLUSTER} nodes (install >10min)"
        phase3_nightly_fix_spoke_nodes && NIGHTLY_FIX_APPLIED=true || true
      fi
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --help|-h)
      ...
      echo "  --branch <BRANCH>        Git branch to use (default: main / openshift-4.20)"
      echo "                             Known branches auto-configure network/image"
      echo "                             Custom branches require --network + --ocp-image"
      ...
      echo ""
      echo "Examples:"
      echo "  $0 --branch openshift-4.22 --clusters etl4"
      echo "  $0 --branch my-feature --network ocp3m0w-ic4n22 --ocp-image img4.22.0-x86-64-appsub"
      echo "  GITOPS_BRANCH=openshift-4.22 OCP_VERSION_IMAGE=img4.22-custom $0"
      exit 0
      ;;
...
echo "  ImageSet:   ${OCP_VERSION_IMAGE}"
echo "  Nightly:    $(is_nightly_build && echo 'yes (workarounds enabled)' || echo 'no')"
